...
Параллельность при работе с прокси вычисляется автоматически: количество прокси × PROXY_MAX_CONNECTIONS. Например, 20 прокси × 3 соединения = 60 одновременных запросов. Параметр `--workers` в этом случае игнорируется — он используется только при работе без прокси.

Перед началом аудита выполняется preflight-проверка: один GET-запрос к целевому сайту через каждый прокси. Прокси, не прошедшие проверку, исключаются из пула до начала работы. Если ни один прокси не работает с указанным сайтом — аудит продолжится без прокси.
...
└── site_audit/
    ...
    ├── proxy.py             # Управление пулом прокси: ротация, cooldown, preflight-проверка
...
| `--workers` | 20 | Параллельность загрузки (без прокси). С прокси вычисляется автоматически: кол-во прокси × PROXY_MAX_CONNECTIONS |
...
| `PROXY_MAX_CONNECTIONS` | 3 | Максимум одновременных соединений на один прокси. Общая параллельность = кол-во прокси × это значение |
...
| `DEFAULT_WORKERS` | 20 | Параллельность загрузки (без прокси). С прокси параллельность вычисляется автоматически |
...
| `DEFAULT_MAX_TOTAL_PAGES` | 0 (без лимита) | Жёсткий лимит общего числа страниц для защиты памяти. 0 = без лимита |
...
## Preflight-проверка прокси

При включённых прокси (`PROXY_ENABLED=true`) перед каждым аудитом автоматически выполняется preflight-проверка:

1. Каждый прокси из пула получает один GET-запрос к целевому сайту
2. Все проверки выполняются параллельно (занимает несколько секунд)
3. Прокси считается рабочим, если сервер ответил с HTTP-кодом < 500
4. Нерабочие прокси сразу отправляются в cooldown и не используются при аудите
5. Если ни один прокси не прошёл проверку — аудит продолжается без прокси (напрямую)

Это позволяет избежать массовых ошибок при аудите сайтов, которые блокируют определённые прокси.

В CLI прогресс отображается в консоли:

🔌 Проверка прокси с сайтом https://mysite.ru... ✅ Прокси проверены: 5/20 работают

Если ни один прокси не прошёл:

🔌 Проверка прокси с сайтом https://mysite.ru... ⚠️ Ни один прокси (20 шт.) не работает с https://mysite.ru. Продолжаю без прокси.

В Telegram-боте результат приходит сообщением в чат.

## Параллельность и прокси
Уровень параллельности HTTP-запросов определяется автоматически:

Режим	Формула	Пример
С прокси	кол-во прокси × PROXY_MAX_CONNECTIONS	20 прокси × 3 = 60 параллельных запросов
Без прокси	DEFAULT_WORKERS	20 параллельных запросов
Единый семафор управляет параллельностью на всех этапах: загрузка страниц, BFS-обход, проверка ссылок, проверка картинок, проверка редиректов.

Рекомендуемые значения PROXY_MAX_CONNECTIONS:

1 — максимальная стабильность, медленная скорость
3 — баланс скорости и стабильности (по умолчанию)
5 — высокая скорость, подходит для качественных прокси
Если при увеличении PROXY_MAX_CONNECTIONS аудит замедляется или появляются таймауты — уменьшите значение.
...
2. Загрузка страниц
   └── Асинхронное скачивание через aiohttp
       ├── С прокси: параллельность = кол-во прокси × PROXY_MAX_CONNECTIONS
       ├── Без прокси: параллельность = DEFAULT_WORKERS
       └── Загрузка порциями по 500 URL для экономии памяти
...
Убедитесь, что в .env указано PROXY_ENABLED=true и файл proxies.txt заполнен. Прокси используются на всех этапах: загрузка страниц, проверка ссылок, проверка картинок, проверка редиректов. Параллельность вычисляется автоматически по количеству прокси — параметр --workers не нужен.
